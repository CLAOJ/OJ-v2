...
import { getRankColor, getRankBadgeColor, getRatingChangeColor, formatRatingChange } from '@/lib/utils';
...
export default function ContestDetailPage({ params }: { params: Promise<{ key: string }> }) {
    ...
    const { data: ranking, isLoading: isRankingFetching } = useQuery({
        ...
        // Auto-refresh scoreboard every 10 seconds when contest is running
    ...
    // Subscribe to contest WebSocket channel for live updates
    useEffect(() => {
        if (!key) return;
                ...
                <div className="p-10 md:p-14 md:pb-8 space-y-8 relative">
                    ...
                    <div className="flex flex-wrap items-center gap-4 pt-4">
                        {isRunning && (
                            ...
                            >
                                {contest.is_joined ? <ShieldCheck size={20} /> : <Play size={20} fill="currentColor" />}
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            <section className="p-10 rounded-[3rem] border bg-card shadow-sm space-y-8 min-h-[40vh]">
                                ...
                                <div className="prose prose-zinc dark:prose-invert max-w-none text-muted-foreground leading-relaxed">
                                    {contest.description || "The contest description will be displayed here. Challenge your algorithmic skills and climb the rankings!"}
                                </div>
                            </section>
                                    ...
                                    <tbody className="divide-y">
                                        {contest.problems.map((prob, idx) => (
                                            <tr key={prob.code} className="group hover:bg-muted/10 transition-colors">
                                                ...
                                                <td className="px-10 py-6 text-center">
                                                    <div className="flex flex-col items-center gap-1.5 focus-within:ring-0">
