...
import { MessageSquare, MessageSquarePlus, ChevronDown, ChevronUp } from 'lucide-react';
...
export function ContestClarificationList({ contestKey, canCreate, onClarificationCreated }: ContestClarificationListProps) {
    ...
    const createMutation = useMutation({
        mutationFn: async (question: string) => {
            await api.post(`/contest/${contestKey}/clarifications`, { body: question });
        },
        onSuccess: () => {
                        ...
                        >
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <Badge variant={clarification.answered ? 'success' : 'warning'}>
                                        {clarification.answered ? 'Answered' : 'Pending'}
                                    </Badge>
                                    <span className="font-mono text-sm">{clarification.problem_code}</span>
                                    <span className="text-muted-foreground text-sm">
                                        {new Date(clarification.time).toLocaleString()}
                                    </span>
                                </div>
